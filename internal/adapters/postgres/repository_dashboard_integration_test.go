...
func TestRefreshCostAnomaliesExecutesAgainstPostgres(t *testing.T) {
	dsn := os.Getenv("CLOUDPULSE_TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set CLOUDPULSE_TEST_DATABASE_URL to run PostgreSQL anomaly SQL regression test")
	}
...
func TestPostgresReportsTimestampComparedToIntervalRegressionMessage(t *testing.T) {
	dsn := os.Getenv("CLOUDPULSE_TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set CLOUDPULSE_TEST_DATABASE_URL to run PostgreSQL timestamp-vs-interval error regression test")
	}
