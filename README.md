...
- AWS CUR/Data Export ingestion, optional AWS Cost Explorer ingestion, and OCI billing export ingestion
...
internal/adapters/providers  cloud collectors, including AWS CUR/S3, AWS Cost Explorer, and OCI Object Storage
...
dashboards                   AWS, OCI, and waste Grafana dashboard JSON
```

General provider documentation is ordered by common cloud adoption: AWS first, then OCI. Provider-specific sections remain focused on that provider.
...
AWS defaults to CUR 2.0 / Data Export files in S3 so it follows the same architecture as other billing exports: bucket export -> Torvix collector -> normalized PostgreSQL records -> dashboards and reports. Cost Explorer remains available as an optional `cost_explorer` mode for quick testing, debugging, or manual fallback.
...
- `GET /api/v1/dashboard/overview?provider=aws|oci`
- `GET /api/v1/dashboard/cost-timeseries?provider=aws|oci&from=YYYY-MM-DD&to=YYYY-MM-DD&window=daily|weekly|monthly`
- `GET /api/v1/dashboard/cost-by-category?provider=aws|oci&from=YYYY-MM-DD&to=YYYY-MM-DD`
- `GET /api/v1/dashboard/cost-by-service?provider=aws|oci&region=<region>&scope=<scope>&service=<service>&from=YYYY-MM-DD&to=YYYY-MM-DD&limit=15`
- `GET /api/v1/dashboard/cost-by-provider?provider=aws|oci&from=YYYY-MM-DD&to=YYYY-MM-DD`
- `GET /api/v1/dashboard/cost-by-compartment?provider=oci&region=<region>&from=YYYY-MM-DD&to=YYYY-MM-DD&limit=15`
- `GET /api/v1/dashboard/cost-by-scope?provider=aws|oci&region=<region>&from=YYYY-MM-DD&to=YYYY-MM-DD&limit=15`
- `GET /api/v1/dashboard/cost-by-region?provider=aws|oci&from=YYYY-MM-DD&to=YYYY-MM-DD`
- `GET /api/v1/dashboard/drilldown?provider=aws|oci&from=YYYY-MM-DD&to=YYYY-MM-DD`
...
The bundled AWS Grafana dashboard drills into cost in this order: Region -> Account/Scope -> Service. The bundled OCI Grafana dashboard drills into cost in this order: Region -> Compartment -> Service. The Region, Scope/Compartment, and Service variables support `All`; when a variable is `All`, the matching API filter is not applied. The dashboards use the selected Grafana time range and show aggregate/top values until filters are selected. `Top OCI Cost Drivers` returns Region, Compartment, Service, Total Cost, and percent of the filtered total.
